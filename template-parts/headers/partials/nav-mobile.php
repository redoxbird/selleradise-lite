...
<ul <?php if ($parent) : ?> x-show="activeChild == true" x-collapse class="m-0 p-0 pl-3 w-full text-sm font-medium border-0 border-l-1 border-solid border-l-zinc-200" <?php else : ?> class="m-0 px-page pb-12 w-full text-md min-h-[30rem]" <?php endif; ?>>
  <?php foreach ($items as $item) : ?>
    <li class="list-none flex justify-between flex-wrap items-center w-full" x-data="{activeChild: '<?php echo $item->activeAncestor ? true : false ?>'}">
      ...
      <?php if ($item->children) : ?>
        <button class="w-8 h-8 flex justify-center items-center py-2 bg-gray-50 border-1 border-gray-300 border-0 rounded-full" x-on:click="activeChild = !activeChild" aria-label="<?php echo esc_html(__("Open", "charlesrwood") . ' ' . $item->label); ?>">
